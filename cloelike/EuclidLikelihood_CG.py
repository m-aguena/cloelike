...
class EuclidLikelihood_CG:
    ...
    def get_CG_theory_vector(self, parameters: dict, settings: dict):
        ...
        profile = self.Profile(
            ...
            two_halo=settings["two_halo"],
            offcentering=settings["offcentering"],
            rms_off=settings["rms_off"],
            f_off=settings["f_off"],
            trunc_fact=settings["trunc_fact"],
        ...
        selectionFunction = self.SelectionFunction(
            ...
            B_l=parameters["B_l"],
            C_l=parameters["C_l"],
            sig_A_l=parameters["sig_A_l"],
            sig_B_l=parameters["sig_B_l"],
            sig_C_l=parameters["sig_C_l"],
            sig_lambda_norm=parameters["sig_lambda_norm"],
            sig_lambda_z=parameters["sig_lambda_z"],
            sig_lambda_exponent=parameters["sig_lambda_exponent"],
            sig_z_z=parameters["sig_z_z"],
            sig_z_lambda=parameters["sig_z_lambda"],
        )

        haloClustering = self.HaloClustering(
            perturbations, perturbations_fid, selectionFunction, k=settings["k"]
        )

        covariance = self.HaloCovariance(
            perturbations,
            area=settings["area"],
        ...
        clusterStatistics = self.ClusterStatistics(
            ...
            Rad_obs_edges=settings["Rad_obs_edges"],
            Lambda_obs_Cxi2_edges=settings["Lambda_obs_Cxi2_edges"],
            Rad_obs_Cxi2_edges=settings["Rad_obs_Cxi2_edges"],
            z_obs_Cxi2_edges=settings["zed_obs_Cxi2_edges"],
            ...
            z=settings["zed"],
            area=settings["area"],
            CG_like_selection=settings["CG_like_selection"],
            CG_xi2_cov_selection=settings["CG_xi2_cov_selection"],
            bias=settings["bias"],
            neutrino_cdm=settings["neutrino_cdm"],
        )
    ...
    def loglike(self, parameters: dict, settings: dict):
        ...
        if settings["CG_like_selection"] == "CC":
            CGCCthvec = self.get_CG_theory_vector(parameters, settings)
            ...
            createCGtheory = self.get_CG_theory_vector(parameters, settings)
